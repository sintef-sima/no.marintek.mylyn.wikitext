...
public class ChartFactory {
	...
	for (int series = 0; series < xSeries.length; series++) {
	    addSeries(titles, ylabel, xlabel, ySeries[series], xSeries[series], dmlchartObjectFactory, valueAxisId,
		    categoryAxisId, dmlObjectFactory, plotarea, linechart, series, series);
	}
    ...
    private static void addSeries(String[] title, String ylabel, String xlabel, double[] ySeries, double[] xSeries,
	    ...
	    org.docx4j.dml.ObjectFactory dmlObjectFactory, CTPlotArea plotarea, CTLineChart linechart, int order,
	    int index) {
	// Create object for dispUnits
	...
	// CTBuiltInUnit builtinunit =
	// dmlchartObjectFactory.createCTBuiltInUnit();
